...
describe('ExperienceList', () => {
      ...
      roles: [
        {
          ...
          skills: [{ id: 's1', name: 'React' }, { id: 's2', name: 'Go' }],
        },
      ],
    },
    {
  ...
  it('renders dates correctly', () => {
    ...
    expect(screen.getByText((content) => content.includes('Jan 2020') && content.includes('Present'))).toBeInTheDocument();
    expect(screen.getByText((content) => content.includes('Jan 2018') && content.includes('Dec 2019'))).toBeInTheDocument();
  });
  ...
  it('highlights skills when requested', () => {
      render(<ExperienceList companies={mockCompanies} highlightSkillIDs={['s1']} />);
