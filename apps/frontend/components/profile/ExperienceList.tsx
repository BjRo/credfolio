...
export default function ExperienceList({
	companies,
	highlightSkillIDs,
}: { companies: CompanyEntry[]; highlightSkillIDs?: string[] }) {
	if (!companies || companies.length === 0)
		return (
			<div className="text-gray-500 italic">No experience recorded yet.</div>
		);
					...
					<div className="md:flex items-center justify-between mb-4 md:mb-0 md:flex-row-reverse group">
						...
						<div className="md:w-1/2 md:pl-8 mb-4 md:mb-0">
							...
							<p className="text-sm text-gray-500">
								{new Date(company.start_date).toLocaleDateString(undefined, {
									year: "numeric",
									month: "short",
								})}{" "}
								-
								{company.end_date
									? new Date(company.end_date).toLocaleDateString(undefined, {
											year: "numeric",
											month: "short",
										})
									: "Present"}
					...
					<div className="mt-4 space-y-4 md:ml-auto md:w-[calc(50%+2rem)] md:pl-8">
						{company.roles?.map((role) => (
							...
							>
								...
								<p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap mb-4 text-sm leading-relaxed">
									{role.description}
								</p>
